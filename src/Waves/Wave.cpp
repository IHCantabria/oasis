...
arma::vec Wave::GetFreeSurface(double time, arma::vec x, arma::vec y)
{
    ...
    arma::vec eta = arma::sum(amplitudes * ones(size(amplitudes.t())) *
                              arma::cos(phases * ones(size(x.t())) + kx * x.t() + ky * y.t() -
                                        time * ang_freqs * ones(size(x.t()))))
                        .t();
    return ramp * eta;
}
...
void IrregularWave::GetWaveSpectrum(void)
{
    ...
    if (specType_flag == 1)
    {
        ...
        for (int ii = 0; ii < num_comps; ii++)
        {
            for (int jj = 0; jj < num_headings; jj++)
            {
                this->amplitudes(ii, jj) =
                    std::sqrt(2.0 * spectral_density(ii) * df * directional_spreading(jj) * dtheta);
            }
        }
...
void IrregularWave::GetJonswapSpectrum(void)
{
    ...
    second_order_moment =
        arma::as_scalar(arma::trapz(this->freqs, arma::pow(this->freqs, 2.0) % this->spectral_density));
}
...
arma::vec IrregularWave::GetJonswapSpectrum(arma::vec freqs, double height, double period, double gamma)
{
    ...
    arma::vec S_PM = 0.3125 * pow(height, 2.0) * pow(fp, 4.0) * arma::pow(freqs, -5.0) %
                     arma::exp(-1.25 * arma::pow(fp / freqs, 4.0));
    if (freqs(0) == 0)
    {
        S_PM(0) = 0.0;
    }
...
void IrregularWave::GetSpreadingFunction(void)
{
    ...
    arma::interp1(wrapToPi(theta - heading), normalized_dir_spr, wrapToPi(headings), directional_spreading, "linear",
                  0.0);
}
...
void IrregularWave::GetCheckedFreeSurface(void)
{
    ...
    else if (readPhases_flag == 1)
    {
        ...
        {
            std::stringstream ss;
            ss << "Error while parsing file:" << wavePhasesFileName
               << "; Unexpected number of components or headings. \n";
            throw ValueError(ss.str());
        }
        else
        {
...
int IrregularWave::CheckPhases(arma::vec t, arma::vec eta)
{
    ...
    if ((t(n - 1) - t(0)) < 1800.0)
    {
        if ((Tmean <= TmeanC * (1 + rel_tol)) && (Tmean >= TmeanC * (1 - rel_tol)) && (Hsig <= HsigC * (1 + rel_tol)) &&
            (Hsig >= HsigC * (1 - rel_tol)))
        {
            flag = 1;
        }
        else
    ...
    {
        if ((Tmean <= TmeanC * (1 + rel_tol)) && (Tmean >= TmeanC * (1 - rel_tol)) && (Hsig <= HsigC * (1 + rel_tol)) &&
            (Hsig >= HsigC * (1 - rel_tol)) && (Hmax <= HmaxC * (1 + rel_tol)) && (Hmax >= HmaxC * (1 - rel_tol)))
        {
            flag = 1;
        }
        else
...
void IrregularWave::CutSpectrumZeros(void)
{
    ...
    arma::uvec ind_rows = arma::find((spectral_density >= factor * arma::as_scalar(arma::mean(spectral_density))) &&
                                     (periods >= min_period));
    arma::uvec ind_cols =
        arma::find(directional_spreading >= factor * arma::as_scalar(arma::mean(directional_spreading)));
...
void IrregularWave::CutPiecesSpectrumZeros(void)
{
    ...
    for (int ii = 0; ii < this->num_pieces; ii++)
    {
        for (int jj = 0; jj < num_comps_piece; jj++)
        {
            ...
            {
                amplitudes_max(jj, kk) = std::max(arma::as_scalar(amplitudes_max(jj, kk)),
                                                  arma::as_scalar(this->amplitudes_piece(ii)(jj, kk)));
            }
        }
    }
    ...
    if (num_headings_piece > 1)
    {
        ...
        for (int ii = 1; ii < ind_cols_aux.n_elem - 1; ii++)
        {
            ...
            if ((amplitudes_max_headings(jj - 1) >= factor * amplitudes_max_mean) ||
                (amplitudes_max_headings(jj + 1) >= factor * amplitudes_max_mean))
            {
