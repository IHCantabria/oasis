...
void Line::qs_Functions(double& ff, double& gg, double& DfDH, double& DfDV, double& DgDH, double& DgDV)
{
    ...
    */
    ...
    if ((floor_flag < 0.0) || (om < 0.0) || (VF - om * L > 0.0))
    {
        ...
        DfDH = L / EA + (tempLg - tempOmLg) / om -
               ((temp + temp2 / tempSq) / (temp + tempSq) - (tempOm + tempOm2 / tempOmSq) / (tempOm + tempOmSq)) / om;
    ...
    else
    {
        ...
        ff = HF * L / (EA) + (HF / om) * tempLg + LmVFpOm +
             CB * om * 0.5 / (EA) * (-LmVFpOm * LmVFpOm + (LmVFpOm - HF / (CB * om)) * (LmVFpOm - HF / (CB * om))) - xF;
        gg = (1.0 / EA) * (VF * L - 0.5 * om * L * L) + (HF / om) * (tempSq - 1.0) - zF;
...
void Line::qs_Solution(void)
{
    ...
    if ((floor_flag < 0.0) || (om < 0.0) || (VF > om * L))
    {
        ...
        for (ii = 0; ii < N; ii = ii + 1)
        {
            ...
            temp2 = (VF - om * L) / HF;
            xc(ii, 0) = HF * s(ii, 0) / EA + (HF / om) * (log(temp1(ii, 0) + sqrt(1.0 + temp1(ii, 0) * temp1(ii, 0))) -
                                                          log(temp2 + sqrt(1.0 + temp2 * temp2)));
            zc(ii, 0) = s(ii, 0) / (EA) * (VF - om * L + 0.5 * om * s(ii, 0)) +
                        (HF / om) * (sqrt(1.0 + temp1(ii, 0) * temp1(ii, 0)) - sqrt(1.0 + temp2 * temp2));
            dxcds(ii, 0) =
                HF / EA + (HF / om) * ((om / HF + om * temp1(ii, 0) / (HF * sqrt(1.0 + temp1(ii, 0) * temp1(ii, 0)))) /
                                       (temp1(ii, 0) + sqrt(1.0 + temp1(ii, 0) * temp1(ii, 0))));
            dzcds(ii, 0) = (VF - om * L + om * s(ii, 0)) / EA +
                           (HF / om) * (om * temp1(ii, 0) / (HF * sqrt(1.0 + temp1(ii, 0) * temp1(ii, 0))));
    ...
    else if (-CB * (VF - om * L) < HF)
    {
        ...
        for (ii = 0; ii < N; ii = ii + 1)
        {
            ...
            {
                xc(ii, 0) = HF * s(ii, 0) / EA +
                            (HF / om) * (log(temp1(ii, 0) + sqrt(1.0 + temp1(ii, 0) * temp1(ii, 0)))) + L - VF / om -
                            0.5 * CB * (VF - om * L) * (VF - om * L) / (om * EA);
                zc(ii, 0) = (HF / om) * (-1.0 + sqrt(1.0 + temp1(ii, 0) * temp1(ii, 0))) +
                            s(ii, 0) / (EA) * (VF - om * L + 0.5 * om * s(ii, 0)) +
                            0.5 * (VF - om * L) * (VF - om * L) / (om * EA);
                dxcds(ii, 0) =
                    HF / EA +
                    (HF / om) * ((om / HF + om * temp1(ii, 0) / (HF * sqrt(1.0 + temp1(ii, 0) * temp1(ii, 0)))) /
                                 (temp1(ii, 0) + sqrt(1.0 + temp1(ii, 0) * temp1(ii, 0))));
                dzcds(ii, 0) = (HF / om) * (om * temp1(ii, 0) / (HF * sqrt(1.0 + temp1(ii, 0) * temp1(ii, 0)))) +
                               (VF - om * L + om * s(ii, 0)) / EA;
                Te(ii, 0) = sqrt(HF * HF + (VF - om * L + om * s(ii, 0)) * (VF - om * L + om * s(ii, 0)));
            }
        }
    }
    else
    {
        ...
        for (ii = 0; ii < N; ii = ii + 1)
        {
            ...
            {
                xc(ii, 0) = s(ii, 0) - (L - (VF / om) - 0.5 * HF / (om * CB)) * (HF / EA) +
                            (s(ii, 0) / EA) * (HF + CB * (VF - om * L) + 0.5 * om * s(ii, 0) * CB) +
                            0.5 * CB * (VF - om * L) * (VF - om * L) / (om * EA);
                zc(ii, 0) = 0.0;
                dxcds(ii, 0) = 1.0 + (HF + CB * (VF - om * L) + om * s(ii, 0) * CB) / EA;
                dzcds(ii, 0) = 0.0;
