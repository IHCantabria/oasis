...
void Sinking::ReadPropertiesASCII(FILE* pFile, std::string inputFolderPath)
{
    ...
    for (int ii = 0; ii < numGroups; ii++)
    {
        ...
        for (int jj = 0; jj < itemp; jj++)
        {
            ...
            groupsIx(ii, 0) = groupsIx(ii, 0) + dtemp * (mtemp(jj, 1) * mtemp(jj, 1) + mtemp(jj, 1) * mtemp(jj + 1, 1) +
                                                         mtemp(jj + 1, 1) * mtemp(jj + 1, 1));
            groupsIy(ii, 0) = groupsIy(ii, 0) + dtemp * (mtemp(jj, 0) * mtemp(jj, 0) + mtemp(jj, 0) * mtemp(jj + 1, 0) +
                                                         mtemp(jj + 1, 0) * mtemp(jj + 1, 0));
...
void Sinking::ReadPropertiesYAML(YAML::Node node, std::string inputFolderPath)
{
    ...
    for (int ii = 0; ii < numGroups; ii++)
    {
        ...
        for (int jj = 0; jj < itemp; jj++)
        {
            ...
            groupsIx(ii, 0) += dtemp * (mtemp(jj, 1) * mtemp(jj, 1) + mtemp(jj, 1) * mtemp(jj + 1, 1) +
                                        mtemp(jj + 1, 1) * mtemp(jj + 1, 1));
            groupsIy(ii, 0) += dtemp * (mtemp(jj, 0) * mtemp(jj, 0) + mtemp(jj, 0) * mtemp(jj + 1, 0) +
                                        mtemp(jj + 1, 0) * mtemp(jj + 1, 0));
...
void Sinking::UpdateSinkingHydrodynamics(double t)
{
    ...
    if (numHDBs > 1)
    {
        ...
        UpdateInterpHydro();
        // std::cout << "        pSinkingBody->pHydro->InterpolateHydro(pHydro[indHydro1], pHydro[indHydro2],
        // hydroInterpCoef);" << std::endl;
...
void Sinking::UpdateGroupsFillingState(double t)
{
    ...
    arma::mat temp = groupsMasses % (groupsHeights % groupsHeights / 3.0 + groupsHeights % centers.col(2) +
                                     centers.col(2) % centers.col(2));
...
{
    if (pfile_FillingCOG_csv)
        fclose(pfile_FillingCOG_csv);
}
...
{
    if (pSim->outputFormat == 1)
        fprintf(pfile_FillingCOG_csv, "%f,%f,%f,%f,%f\n", t, groupsCOG(0, 0), groupsCOG(1, 0), groupsCOG(2, 0),
                totalFillingMass);
    else
