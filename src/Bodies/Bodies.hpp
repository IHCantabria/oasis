...
public:
    ...
    BCP** pBodyBcps;              // Array of pointers to the boundary condition points
    WindTurbine** pBodyWindTurbs; // Array of pointers to the wind turbine objects
    ...
    HydroForce* pHydro;           // Hydrodynamics body associated to the body
    ...
    arma::mat B_visc2 = arma::zeros(6, 1); // viscous quadratic damping coefficients
    ...
    arma::mat aMat = arma::zeros(3, 3); // Transformation matrix from RPY derivatives to angular velocity vector
    arma::mat aMat_dot = arma::zeros(
        3, 3); // Time derivative of the transformation matrix from RPY derivatives to angular velocity vector
    double structuralMass = 0;               // structural mass
    arma::mat acc = arma::zeros(6, 1);       // Body acceleration w.r.t the global reference system
    arma::mat bcpForces = arma::zeros(6, 1); // Forces exerted by the BCPs on the body, in the COG reference frame
    arma::mat hydrostaticForces = arma::zeros(6, 1); // Storage for the hydrostatic forces
    arma::mat inertia = arma::zeros(6, 6);           // Inertia matrix of the body
    arma::mat invRotMat = arma::zeros(3, 3);         // Inverse rotation matrix
    ...
    arma::mat rotMat = arma::zeros(3, 3);             // Rotation matrix
    arma::mat rotMat_dot = arma::eye(3, 3);           // Time derivative of the body rotation matrix
    arma::mat rotMat_dot2 = arma::eye(3, 3);          // Second time derivative of the body rotation matrix
    arma::mat vel = arma::zeros(6, 1);                // Body velocity w.r.t the global reference system
    arma::mat velBuffer; // Velocity Buffer (global coords) in order to store the body velocities and calculate
                         // Duhamel's integral term
    ...
    // Methods definition
    Body(void) {};
    Body(int n, Simulation* pSim);    // Initialises a body object with the given index
    void ComputeBcpForces(void);      // Computes the resultant force/moment at the COG from all BCP forces
    void ComputeWindTurbForces(void); // Computes the resultant force at the COG from all wind turbine forces
    int GetId(void);                  // Returns the body identification number
    void LoadHydrodynamicDatabase(Body** hydroDatabaseBodies,
                                  int slotIndex); // Loads the hydrodynamics database associated, if any
    void ReadPropertiesASCII(FILE* filePointer);  // Read body properties from ASCII input file
    ...
    void UpdateBcps(void);                    // Update BCP position and velocity values
