...
arma::mat HydroDatabase::CalculateHydrostaticForces(double time)
{
    ...
    if (pBodies[idBody]->flag_hydrostatics == 0)
    {
        ...
        if (pBodies[idBody]->filling_mass > 0)
        {
            ...
            hydrostatic_force.rows(3, 5) = hydrostatic_force.rows(3, 5) + arma::cross(pBodies[idBody]->pos_filling_cog,
                                                                                      pBodies[idBody]->rotMat.t() * Fg);
        }
    }
    else if (pBodies[idBody]->flag_hydrostatics > 0)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            hydrostatic_force(i + 3, 0) = arma::dot(
                -pressure, (radius.col(i1) % weightsJacNormal.col(i2) - radius.col(i2) % weightsJacNormal.col(i1)));
        }
...
void HydroDatabase::ComputeIRF(std::string HDBname)
{
    ...
    frequencies_trapz = arma::regspace(std::max((*pFrequencies).min(), df), df,
                                       std::min(1.0 / (2.0 * pSim->hydroTimeStep), (*pFrequencies).max()))
                            .t();
        ...
        for (int i = 0; i < 6; i++)
        {
            for (int j = 0; j < 6; j++)
            {
                ...
                for (int k = 1; k < IRFTime.n_cols; k++)
                {
                    ...
                    (*pIRF[ib])(k, i, j) =
                        2 * trapzi(2 * arma::datum::pi * frequencies_trapz, dummy_mat) / arma::datum::pi;
                }
            }
        }
    }
    ...
    for (int ib = 0; ib < numBodies; ib++)
    {
        ...
        filename = filename + "_IRF_Body_" + std::to_string(pBodies[idBody]->GetId()) + "_fromBody_" +
                   std::to_string(pBodies[ib]->GetId()) + ".dat";
...
void HydroDatabase::ComputeTotalMass(void)
{
    ...
    (*pTotalMass)(arma::span(0, 5), arma::span(6 * (pBodies[idBody]->hydroDatabaseIndex),
                                               6 * (pBodies[idBody]->hydroDatabaseIndex + 1) - 1)) +=
...
HydroDatabase::HydroDatabase(int incId, int incIdBody, Body** incBody, Simulation* pIncSim)
    : HydroForce()
{
    id = incId;
    pBodies = incBody;
...
void HydroDatabase::LoadHydroDataEHYDB(std::string filePath)
{
    ...
    {
        std::stringstream ss;
        ss << " The number of bodies in dataBodies.dat does not match the number of bodies in the multibody "
              "hydrodatabase."
           << std::endl;
...
void HydroDatabase::LoadHydroDataH5(std::string filePath)
{
        ...
        for (int j = 0; j < numFrequencies; j++)
        {
            for (int k = 0; k < 6; k++)
            {
                for (int l = 0; l < 6; l++)
                {
                    ...
                    int index = i * numBodies * numFrequencies * 6 * 6 + idBody * numFrequencies * 6 * 6 + j * 6 * 6 +
                                k * 6 + l;
    ...
    diffractionForceMagDataset.read(buffer_diffraction_force_mag, H5::PredType::NATIVE_DOUBLE, diffractionForceMagSpace,
                                    diffractionForceMagSpace);
    diffractionForcePhaDataset.read(buffer_diffraction_force_pha, H5::PredType::NATIVE_DOUBLE, diffractionForcePhaSpace,
                                    diffractionForcePhaSpace);
    froudeKrylovForceMagDataset.read(buffer_froude_krylov_force_mag, H5::PredType::NATIVE_DOUBLE,
                                     froudeKrylovForceMagSpace, froudeKrylovForceMagSpace);
    froudeKrylovForcePhaDataset.read(buffer_froude_krylov_force_pha, H5::PredType::NATIVE_DOUBLE,
                                     froudeKrylovForcePhaSpace, froudeKrylovForcePhaSpace);
    waveExcitingMagDataset.read(buffer_wave_exciting_mag, H5::PredType::NATIVE_DOUBLE, waveExcitingMagSpace,
                                waveExcitingMagSpace);
    waveExcitingPhaDataset.read(buffer_wave_exciting_pha, H5::PredType::NATIVE_DOUBLE, waveExcitingPhaSpace,
                                waveExcitingPhaSpace);
    // Close the datasets
    ...
    hydrostaticStiffnessDataset.read(buffer_hydrostatic_stiffness, H5::PredType::NATIVE_DOUBLE,
                                     hydrostaticStiffnessSpace, hydrostaticStiffnessSpace);
    ...
    else
    {
        if (pBodies[idBody]->secondOrderExcitationFlag > 0 && pBodies[idBody]->secondOrderExcitationFlag < 4)
        {
            ...
            // Allocate memory for the QTF buffers [numBodies, numHeadings, numHeadings, numFrequencies, numFrequencies,
            // 6]
                ...
                for (int idof = 0; idof < 6; idof++)
                {
                    ...
                    for (int if1 = 0; if1 < numFrequencies; if1++)
                    {
                        for (int if2 = 0; if2 < numFrequencies; if2++)
                        {
                            ...
                            {
                                // Calculate the index for the 1D buffer [numBodies, numHeadings, numHeadings,
                                // numFrequencies, numFrequencies, 6]
                                int ind = idBody * numHeadings * numHeadings * numFrequencies * numFrequencies * 6 +
                                          ih * numHeadings * numFrequencies * numFrequencies * 6 +
                                          ih * numFrequencies * numFrequencies * 6 + if1 * numFrequencies * 6 +
                                          if2 * 6 + idof;
                                // Copy data from buffer to arrays
                                ...
                                if (ipart == 0)
                                {
                                    (*pQtfDiff[ipart][idof])(if1, if2, ih) =
                                        buffer_qtf_diff_mag[ind] * cos(buffer_qtf_diff_pha[ind]);
                                    (*pQtfSum[ipart][idof])(if1, if2, ih) =
                                        buffer_qtf_sum_mag[ind] * cos(buffer_qtf_sum_pha[ind]);
                                }
                                else
                                {
                                    (*pQtfDiff[ipart][idof])(if1, if2, ih) =
                                        buffer_qtf_diff_mag[ind] * sin(buffer_qtf_diff_pha[ind]);
                                    (*pQtfSum[ipart][idof])(if1, if2, ih) =
                                        buffer_qtf_sum_mag[ind] * sin(buffer_qtf_sum_pha[ind]);
                                }
                            }
                        }
                    }
    ...
    else
    {
        ...
        meanDriftMagDataset.read(buffer_mean_drift_mag, H5::PredType::NATIVE_DOUBLE, meanDriftMagSpace,
                                 meanDriftMagSpace);
        // Close the dataset
        ...
        for (int ihd = 0; ihd < numHeadings; ihd++)
        {
            for (int ifr = 0; ifr < numFrequencies; ifr++)
            {
                ...
                {
                    // Calculate the index for the 1D buffer [numHeadings, numBodies, numFrequencies, 6]
                    int ind = ihd * numBodies * numFrequencies * 6 + idBody * numFrequencies * 6 + ifr * 6 + idof;
                    // Copy data from buffer to arrays
...
arma::mat HydroDatabase::ComputeFirstWaveExcForce(double t)
{
    ...
    for (int ii = 0; ii < activeDofs; ii++)
    {
        ...
        Cm = arma::sum(
            H_Mag_loc % amplitudes_w(ind_piece) % arma::cos(phases_w(ind_piece) + H_Pha_loc + x * kx_w + y * ky_w), 1);
        Sm = arma::sum(
            H_Mag_loc % amplitudes_w(ind_piece) % arma::sin(phases_w(ind_piece) + H_Pha_loc + x * kx_w + y * ky_w), 1);
        ...
        if (flag_gap)
        {
            ...
            Cm = arma::sum(H_Mag_loc % amplitudes_w(ind_piece - 1) %
                               arma::cos(phases_w(ind_piece - 1) + H_Pha_loc + x * kx_w + y * ky_w),
                           1);
            Sm = arma::sum(H_Mag_loc % amplitudes_w(ind_piece - 1) %
                               arma::sin(phases_w(ind_piece - 1) + H_Pha_loc + x * kx_w + y * ky_w),
                           1);
...
arma::mat HydroDatabase::ComputeSecondWaveExcForce(double t)
{
    ...
    for (int ii = 0; ii < activeDofs; ii++)
    {
        ...
        temp_mat_2 = (*HDif[0][ii]) % ampP(ind_piece) % arma::cos(temp_mat_1) -
                     (*HDif[1][ii]) % ampP(ind_piece) % arma::sin(temp_mat_1);
        ...
        if (flag_gap)
        {
            ...
            temp_mat_2 = (*HDif[0][ii]) % ampP(ind_piece - 1) % arma::cos(temp_mat_1) -
                         (*HDif[1][ii]) % ampP(ind_piece - 1) % arma::sin(temp_mat_1);
...
void HydroDatabase::SetUp(void)
{
    ...
    // std::cout << "HydroDatabase::SetUp - Convert from real/imag to mag/pha the first order transfer functions" <<
    // std::endl; Convert from real/imag to mag/pha the first order transfer functions
    arma::cube WE_Real = (*pWaveExcitingMag) % arma::cos((*pWaveExcitingPha));
    arma::cube WE_Imag = (*pWaveExcitingMag) % arma::sin((*pWaveExcitingPha));

    // Check that the transfer functions frequencies cover the wave frequencies, return a warning and crop frequencies
    // outside if not
    ...
    // std::cout << "HydroDatabase::SetUp - Interpolate the first order transfer functions to the wave frequencies" <<
    // std::endl; Interpolate the first order transfer functions to the wave frequencies
    ...
    if (pBodies[idBody]->secondOrderExcitationFlag > 0 && pBodies[idBody]->secondOrderExcitationFlag < 4)
    {
        // std::cout << "HydroDatabase::SetUp - Interpolate the second order transfer functions to the wave frequencies"
        // << std::endl; Interpolate the second order transfer functions to the wave frequencies
        arma::cube temp;
        ...
        // std::cout << "HydroDatabase::SetUp - Define the matrices required for time domain QTF forces computation" <<
        // std::endl; Preprocess the matrices required for time domain QTF forces computation
        ...
        // std::cout << "HydroDatabase::SetUp - Preprocess the matrices required for time domain QTF forces computation
        // (w, k)" << std::endl;
        int i_wave, j_wave;
    ...
    if (pBodies[idBody]->secondOrderExcitationFlag > 2)
    {
        ...
        // TODO: Review this!
        arma::cube temp_mD =
            interp2(*pFrequencies, *pHeadings, permute(*pMeanDrift, 231), freqs_w, pWave->headings_piece);
        for (int ii = 0; ii < activeDofs; ii++)
        {
...
void HydroDatabase::InterpolateHydro(HydroDatabase* pHydro1, HydroDatabase* pHydro2, double interpCoef)
{
    ...
    (*pHydrostaticStiffness) =
        *(pHydro1->pHydrostaticStiffness) * (1 - interpCoef) + *(pHydro2->pHydrostaticStiffness) * interpCoef;
    ...
    for (int ii = 0; ii < numBodies; ii++)
    {
        ...
        *pDampingRadiation[ii] =
            *(pHydro1->pDampingRadiation[ii]) * (1 - interpCoef) + *(pHydro2->pDampingRadiation[ii]) * interpCoef;
        *pDampingRadiationLf[ii] =
            *(pHydro1->pDampingRadiationLf[ii]) * (1 - interpCoef) + *(pHydro2->pDampingRadiationLf[ii]) * interpCoef;
    ...
    if (pBodies[idBody]->secondOrderExcitationFlag > 0 && pBodies[idBody]->secondOrderExcitationFlag < 4)
    {
        for (int ii = 0; ii < 2; ii++)
        {
            ...
            {
                *pQtfDiff[ii][jj] =
                    *(pHydro1->pQtfDiff[ii][jj]) * (1 - interpCoef) + *(pHydro2->pQtfDiff[ii][jj]) * interpCoef;
                *pQtfSum[ii][jj] =
                    *(pHydro1->pQtfSum[ii][jj]) * (1 - interpCoef) + *(pHydro2->pQtfSum[ii][jj]) * interpCoef;
            }
        }
    }
