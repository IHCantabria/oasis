...
public:
    ...
    int numFrequencies;
    int numHeadings;
    arma::cube*** pQtfDiff; // Matrix components: [parts, dofs, freq1, freq2, headings] | The first two are pointers to
                            // cube variables and last three are arma::cube
    arma::cube*** pQtfSum;  // Matrix components: [parts, dofs, freq1, freq2, headings] | The first two are pointers to
                            // cube variables and last three are arma::cube
    ...
    arma::cube*** QtfDiff_w; // Matrix components after SetUp: [parts, dofs, freq1_w, freq2_w, headings] | The first two
                             // are pointers to cube variables and last three are arma::cube
    arma::cube*** QtfSum_w;  // Matrix components after SetUp: [parts, dofs, freq1_w, freq2_w, headings] | The first two
                             // are pointers to cube variables and last three are arma::cube
    arma::mat*** QtfDiff_wt; // Matrix components after SetUp: [parts, dofs, freq1_w, freq2_w] (heading_w) | The first
                             // two are pointers to mat variables and last two are arma::mat
    arma::mat*** QtfSum_wt;  // Matrix components after SetUp: [parts, dofs, freq1_w, freq2_w] (heading_w) | The first
                             // two are pointers to mat variables and last two are arma::mat
    ...
    int numFrequencies_w;    // Number of frequencies in the wave spectrum inside the frequency range of hydrodynamic
    ...
    arma::mat
    GetTotalMass(void); // Interface method, it returns the total mass matrix: StruturalMass+AddedMass+ViscousAddedMass
    void UpdateStructuralMass(arma::mat newStructuralMass);
    void UpdateTotalMass(void);
    arma::mat
    ComputeFirstWaveExcForce(double t); // Interpolate First Order Wave Excitation forces using first order polynomial
