...
void Spring::leer_datosSprings(void){
	...
	for(ii=0;ii<2;ii=ii+1){
		for(jj=0;jj<6;jj=jj+1){
			...
			arma::mat temp_vec2(temp_N,1)
			arma::mat temp_mat(temp_N,6)
			temp_vec2 = 0.0;
...
void Spring::computeSpringForces(void){
	...
	arma::field<mat> SpringVectorsG(3,2);
	...
	arma::mat SpringStrains(6,2); // Deformacion en el muelle
	arma::mat SpringStrains_dot(6,2); // Derivada temporal de la deformacion en el muelle
	for(int ii=0;ii<2;ii=ii+1){
		for(int jj=0;jj<3;jj=jj+1){
			...
			if(dampingFlag == 1){
				SpringStrains_dot(jj,ii) = arma::dot(SpringVectorG_dot.rows(0,2),SpringVectorsG(jj,ii)) - (2*(ii == 0)-1)*L*(jj == 0);
				SpringStrains_dot(jj+3,ii) = SpringVectorG_dot(jj+3,0);
			}
		}
	}

	arma::mat tempF_L(6,1);
	arma::mat tempF_G(6,1);
	double tempS;
	int tempN;
	int tempI;
	...
	for(int ii=0;ii<2;ii=ii+1){
		...
		if(stressModelFlag==1){
			...
			for(int jj=0;jj<6;jj=jj+1){
				...
				if((tempS<=arma::max(temp_strainData))&&(tempS>=arma::min(temp_strainData))){

					tempI = arma::as_scalar(arma::find(temp_strainData>=tempS,1,"first")) - 1;
					dS = tempS - temp_strainData(tempI-1,0);
				...
				} else if (tempS>arma::max(temp_strainData)){ 
				...
				}else if (tempS<arma::min(temp_strainData)){
