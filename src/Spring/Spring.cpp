...
void Spring::ReadPropertiesASCII(FILE* pFilePointer, SpringType** pTypes, int numTypes)
{
    ...
    for (jj = 0; jj < 2; jj++)
    {
        for (ii = 0; ii < 3; ii++)
        {
            ...
            fscanf(pFilePointer, "%lf %lf %lf %[^\n]\n", &temp_vec(0, 0), &temp_vec(1, 0), &temp_vec(2, 0),
                   buffer_line);
...
// Computes spring forces at each BCP and stores them in the BCP objects
void Spring::computeSpringForces(void)
{
    // Compute unit vectors of the spring in the global frame for each BCP
    ...
    { // Loop over the two BCPs
        for (int ii = 0; ii < 3; ii = ii + 1)
        { // Loop over the three spring vectors
    ...
    if (BCP_1_type > 0 && flagStickSlip == 0)
    {
        ...
        if (BCP_1_type == 1)
        {
            ...
            SpringBCP[0]->posG_BCP = temp_p0 + arma::as_scalar(temp_v.t() * (temp_p - temp_p0)) *
                                                   temp_v; // Update BCP1 position with projection on line
        }
        else if (BCP_1_type == 2)
        {
            temp_v = SpringVectorsG(0, 0); // BCP1 x vector taken as plane normal vector
            SpringBCP[0]->posG_BCP = temp_p - arma::as_scalar(temp_v.t() * (temp_p - temp_p0)) *
                                                  temp_v; // Update BCP1 position with projection on plane
        }
    ...
    if (BCP_2_type > 0 && flagStickSlip == 0)
    {
        ...
        if (BCP_2_type == 1)
        {
            ...
            SpringBCP[1]->posG_BCP = temp_p0 + arma::as_scalar(temp_v.t() * (temp_p - temp_p0)) *
                                                   temp_v; // Update BCP2 position with projection on line
        }
        else if (BCP_2_type == 2)
        {
            temp_v = SpringVectorsG(0, 1); // BCP2 x vector taken as plane normal vector
            SpringBCP[1]->posG_BCP = temp_p - arma::as_scalar(temp_v.t() * (temp_p - temp_p0)) *
                                                  temp_v; // Update BCP2 position with projection on plane
        }
    ...
    if (frameFlag == 0)
    {
        ...
        mat_spring2global_dot =
            0.5 * (arma::join_horiz(x1_dot, y1_dot, z1_dot) + arma::join_horiz(x2_dot, y2_dot, z2_dot));
    }
    else if (frameFlag == 1)
    {
    ...
    arma::mat SpringStrains = arma::zeros(6, 1);     // Spring deformation vector
    arma::mat SpringStrains_dot = arma::zeros(6, 1); // Time derivative of spring deformation

    SpringStrains.rows(0, 2) = mat_global2spring * L12;
    SpringStrains(3, 0) =
        0.5 * (atan2(arma::dot(y2, z1), arma::dot(z2, z1)) - atan2(arma::dot(y1, z2), arma::dot(z2, z1)));
    SpringStrains(4, 0) = 0.5 * (asin(arma::dot(-x2, z1)) - asin(arma::dot(-x1, z2)));
    SpringStrains(5, 0) =
        0.5 * (atan2(arma::dot(x2, y1), arma::dot(x2, x1)) - atan2(arma::dot(x1, y2), arma::dot(x2, x1)));

    // Same computation for the time derivative
    if (dampingFlag > 0 || frictionFlag > 0)
    {
        SpringStrains_dot.rows(0, 2) = mat_global2spring_dot * L12 + mat_global2spring * L12_dot;
        SpringStrains_dot(3, 0) = 0.5 * (((arma::dot(y2_dot, z1) + arma::dot(y2, z1_dot)) * arma::dot(z2, z1) +
                                          arma::dot(y2, z1) * (arma::dot(z2_dot, z1) + arma::dot(z2, z1_dot))) /
                                             (pow(arma::dot(z2, z1), 2) + pow(arma::dot(y2, z1), 2)) -
                                         ((arma::dot(y1_dot, z2) + arma::dot(y1, z2_dot)) * arma::dot(z1, z2) +
                                          arma::dot(y1, z2) * (arma::dot(z1_dot, z2) + arma::dot(z1, z2_dot))) /
                                             (pow(arma::dot(z1, z2), 2) + pow(arma::dot(y1, z2), 2)));
        SpringStrains_dot(4, 0) =
            0.5 * ((arma::dot(x2_dot, z1) + arma::dot(x2, z1_dot)) / sqrt(1.0 - pow(arma::dot(x2, z1), 2)) -
                   (arma::dot(x1_dot, z2) + arma::dot(x1, z2_dot)) / sqrt(1.0 - pow(arma::dot(x1, z2), 2)));
        SpringStrains_dot(5, 0) = 0.5 * (((arma::dot(x2_dot, y1) + arma::dot(x2, y1_dot)) * arma::dot(x2, x1) +
                                          arma::dot(x2, y1) * (arma::dot(x2_dot, x1) + arma::dot(x2, x1_dot))) /
                                             (pow(arma::dot(x2, y1), 2) + pow(arma::dot(x2, x1), 2)) -
                                         ((arma::dot(x1_dot, y2) + arma::dot(x1, y2_dot)) * arma::dot(x1, x2) +
                                          arma::dot(x1, y2) * (arma::dot(x1_dot, x2) + arma::dot(x1, x2_dot))) /
                                             (pow(arma::dot(x1, y2), 2) + pow(arma::dot(x1, x2), 2)));
    }

    arma::mat tempF_L = arma::zeros(6, 1); // Spring force in local (spring) coordinates
    arma::mat tempF_G = arma::zeros(6, 1); // Spring force in global coordinates
    ...
    if (frictionFlag == 1)
    {
        ...
        for (int jj = 0; jj < 6; jj = jj + 1)
        {
            ...
            {
                mu_vec(jj, 0) =
                    arma::as_scalar(a_1(0, 0) * pow(vv, 3) + a_1(1, 0) * pow(vv, 2) + a_1(2, 0) * vv + a_1(3, 0));
            }
            else if (vv >= vt && vv < vt * 2)
            {
                mu_vec(jj, 0) =
                    arma::as_scalar(a_2(0, 0) * pow(vv, 3) + a_2(1, 0) * pow(vv, 2) + a_2(2, 0) * vv + a_2(3, 0));
            }
            else
            {
                mu_vec(jj, 0) = mu_d;
