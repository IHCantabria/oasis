...
//
{
  ...
  for (i = 1; i <= order; i++)
  {
    ...
    else if (i == order)
    {
      ...
      r2 = 1.0 /
           (1.0 + 0.22 * ((double)(order)-8.0) / (double)(order));
...
//
{
  ...
  for (i = 1; i < n; i++)
  {
    ...
    bj[i] = 4.0 * i_r8 * (i_r8 + alpha) * (i_r8 + beta) * (i_r8 + alpha + beta) / ((abi - 1.0) * (abi + 1.0) * abi * abi);
  }
...
//
{
  ...
  value = tgamma(1.0 + c) * (s * tgamma(1.0 + beta) * value1 / tgamma(2.0 + beta + c) + tgamma(1.0 + alpha) * value2 / tgamma(2.0 + alpha + c));
...
//
{
  ...
  for (i = 1; i <= order; i++)
  {
    ...
    {
      c[i - 1] = 4.0 * (double)(i - 1) * (alpha + (double)(i - 1)) * (beta + (double)(i - 1)) * (alpha + beta + (double)(i - 1)) /
                 ((alpha + beta + (double)(2 * i - 1)) * pow(alpha + beta + (double)(2 * i - 2), 2) * (alpha + beta + (double)(2 * i - 3)));
    }
  }
...
//
{
  ...
  do
  {
    ...
    for (j = 2; j <= n - 1; j++)
    {
      for (i = 0; i < n; i++)
      {
        p[i + j * n] = ((double)(2 * j - 1) * x[i] * p[i + (j - 1) * n] + (double)(-j + 1) * p[i + (j - 2) * n]) / (double)(j);
      }
    }
...
//
{
  ...
  do
  {
    ...
    for (i = 1; i < n; i++)
    {
      x[i] = xold[i] - ((1.0 - xold[i]) / (double)(n)) * (p[i + (n - 1) * n] + p[i + n * n]) / (p[i + (n - 1) * n] - p[i + n * n]);
    }
    test = 0.0;
    for (i = 0; i < n; i++)
    {
