...
//
{
    ...
    for (i = 0; i < n; i++)
    {
        x[i] = cos(r8_pi * (double)(2 * n - 1 - 2 * i) / (double)(2 * n));
    }
...
//
{
    ...
    {
        ab = alpha * 2.0;
        zemu = pow(2.0, ab + 1.0) * pow(r8_gamma(alpha + 1.0), 2) / r8_gamma(ab + 2.0);
    ...
    else if (kind == 4)
    {
        ab = alpha + beta;
        abi = 2.0 + ab;
        zemu = pow(2.0, ab + 1.0) * r8_gamma(alpha + 1.0) * r8_gamma(beta + 1.0) / r8_gamma(abi);
        aj[0] = (beta - alpha) / abi;
    ...
    {
        ab = alpha + beta;
        zemu = r8_gamma(alpha + 1.0) * r8_gamma(-(ab + 1.0)) / r8_gamma(-beta);
        apone = alpha + 1.0;
        aba = ab * apone;
        aj[0] = -apone / (ab + 2.0);
...
//
{
    ...
    for (i = 1; i <= n; i++)
    {
        theta[i - 1] = (double)(i - 1) * r8_pi / (double)(n - 1);
    }
    ...
    for (i = 0; i < n; i++)
    {
        ...
        for (j = 1; j <= ((n - 1) / 2); j++)
        {
            ...
            w[i] = w[i] - b * cos(2.0 * (double)(j)*theta[i]) / (double)(4 * j * j - 1);
        }
    }
...
//
{
    ...
    for (i = 1; i <= n; i++)
    {
        theta[i - 1] = (double)(2 * (n - i) + 1) * r8_pi / (double)(2 * n);
    }
    ...
    for (i = 0; i < n; i++)
    {
        ...
        for (j = 1; j <= (n / 2); j++)
        {
            w[i] = w[i] - 2.0 * cos(2.0 * (double)(j)*theta[i]) / (double)(4 * j * j - 1);
        }
    }
...
//
{
    ...
    for (i = 1; i <= n; i++)
    {
        theta[i - 1] = (double)(n + 1 - i) * r8_pi / (double)(n + 1);
    }
...
//
{
    ...
    value = tgamma(1.0 + c) * 2.0 * tgamma(1.0 + alpha) * value1 / tgamma(2.0 + alpha + c);
...
void gegenbauer_ss_compute(int order, double alpha, double xtab[], double weight[])
...
//
{
    ...
    for (i = 3; i <= order; i++)
    {
        c[i - 1] = (double)(i - 1) * (alpha + alpha + (double)(i - 1)) /
                   ((alpha + alpha + (double)(2 * i - 1)) * (alpha + alpha + (double)(2 * i - 3)));
    }

    delta = tgamma(alpha + 1.0) * tgamma(alpha + 1.0) / tgamma(alpha + alpha + 2.0);
    ...
    for (i = 1; i <= order; i++)
    {
        if (i == 1)
        {
            ...
            r1 = (1.0 + alpha) * (2.78 / (4.0 + (double)(order * order)) + 0.768 * an / (double)(order));
        ...
        else if (i == 2)
        {
            ...
            r2 = 1.0 + 0.06 * ((double)(order)-8.0) * (1.0 + 0.12 * alpha) / (double)(order);

            r3 = 1.0 + 0.012 * alpha * (1.0 + 0.25 * fabs(alpha)) / (double)(order);
        ...
        else if (i == 3)
        {
            ...
            r2 = 1.0 + 0.22 * ((double)(order)-8.0) / (double)(order);

            r3 = 1.0 + 8.0 * alpha / ((6.28 + alpha) * (double)(order * order));
        ...
        else if (i == order - 1)
        {
            ...
            r2 = 1.0 / (1.0 + 0.639 * ((double)(order)-4.0) / (1.0 + 0.71 * ((double)(order)-4.0)));

            r3 = 1.0 / (1.0 + 20.0 * alpha / ((7.5 + alpha) * (double)(order * order)));
        ...
        else if (i == order)
        {
            ...
            r2 = 1.0 / (1.0 + 0.22 * ((double)(order)-8.0) / (double)(order));

            r3 = 1.0 / (1.0 + 8.0 * alpha / ((6.28 + alpha) * (double)(order * order)));
...
//
{
    ...
    gen_laguerre_ss_compute(order_laguerre, alpha_laguerre, x_laguerre, w_laguerre);
...
//
{
    ...
    for (i = 0; i < order; i++)
    {
        ...
        if (i == 0)
        {
            xval = (1.0 + alpha) * (3.0 + 0.92 * alpha) / (1.0 + 2.4 * (double)(order) + 1.8 * alpha);
        }
        else if (i == 1)
        {
            xval = xval + (15.0 + 6.25 * alpha) / (1.0 + 0.9 * alpha + 2.5 * (double)(order));
        }
        else
        {
            r1 = (1.0 + 2.55 * (double)(i - 1)) / (1.9 * (double)(i - 1));

            r2 = 1.26 * (double)(i - 1) * alpha / (1.0 + 3.5 * (double)(i - 1));
...
void gen_laguerre_ss_recur(double* p2, double* dp2, double* p1, double x, int order, double alpha, double b[],
                           double c[])
...
//
{
    ...
    cc = sqrt(r8_pi) * tgamma((double)(order)) / pow(2.0, order - 1);
...
//
{
    ...
    //  Define the zero-th moment.
    //
    zemu = pow(2.0, alpha + beta + 1.0) * r8_gamma(alpha + 1.0) * r8_gamma(beta + 1.0) / r8_gamma(2.0 + alpha + beta);
    //
    //  Define the Jacobi matrix.
    //
    bj = new double[n];
    ...
    bj[0] = 4.0 * (1.0 + alpha) * (1.0 + beta) / ((3.0 + alpha + beta) * (2.0 + alpha + beta) * (2.0 + alpha + beta));
    ...
    for (i = 1; i < n; i++)
    {
        ...
        bj[i] = 4.0 * i_r8 * (i_r8 + alpha) * (i_r8 + beta) * (i_r8 + alpha + beta) /
...
//
{
    ...
    value = tgamma(1.0 + c) * (s * tgamma(1.0 + beta) * value1 / tgamma(2.0 + beta + c) +
                               tgamma(1.0 + alpha) * value2 / tgamma(2.0 + alpha + c));
...
//
{
    ...
    for (i = 1; i <= order; i++)
    {
        ...
        else
        {
            b[i - 1] = (alpha + beta) * (beta - alpha) /
                       ((alpha + beta + (double)(2 * i)) * (alpha + beta + (double)(2 * i - 2)));
        }
        ...
        {
            c[i - 1] = 4.0 * (double)(i - 1) * (alpha + (double)(i - 1)) * (beta + (double)(i - 1)) *
                       (alpha + beta + (double)(i - 1)) /
                       ((alpha + beta + (double)(2 * i - 1)) * pow(alpha + beta + (double)(2 * i - 2), 2) *
                        (alpha + beta + (double)(2 * i - 3)));
        }
    }

    delta = exp(lgamma(alpha + 1.0) + lgamma(beta + 1.0) - lgamma(alpha + beta + 2.0));
    prod = 1.0;
    for (i = 2; i <= order; i++)
    {
        prod = prod * c[i - 1];
    ...
    for (i = 1; i <= order; i++)
    {
        ...
        else if (i == 2)
        {
            ...
            r3 = 1.0 + 0.012 * beta * (1.0 + 0.25 * fabs(alpha)) / (double)(order);
        ...
        else if (i == 3)
        {
            ...
            r3 = 1.0 + 8.0 * beta / ((6.28 + beta) * (double)(order * order));
...
void jacobi_ss_recur(double* p2, double* dp2, double* p1, double x, int order, double alpha, double beta, double b[],
                     double c[])
...
//
{
    ...
    for (i = 0; i < n; i++)
    {
        bj[i] = (double)((i + 1) * (i + 1)) / (double)(4 * (i + 1) * (i + 1) - 1);
        bj[i] = sqrt(bj[i]);
    }
...
//
{
    ...
    *dp2 = 1.0;
    ...
    for (i = 2; i <= order; i++)
    {
        ...
        *p2 = ((double)(2 * i - 1) * x * (*p1) + (double)(-i + 1) * p0) / (double)(i);

        *dp2 = ((double)(2 * i - 1) * ((*p1) + x * dp1) - (double)(i - 1) * dp0) / (double)(i);
    }
...
//
{
    ...
    do
    {
        ...
        for (j = 2; j <= n - 1; j++)
        {
            ...
            {
                p[i + j * n] =
                    ((double)(2 * j - 1) * x[i] * p[i + (j - 1) * n] + (double)(-j + 1) * p[i + (j - 2) * n]) /
                    (double)(j);
            }
        }
        ...
        for (i = 0; i < n; i++)
        {
            x[i] = xold[i] - (x[i] * p[i + (n - 1) * n] - p[i + (n - 2) * n]) / ((double)(n)*p[i + (n - 1) * n]);
        }
...
//
{
    ...
    else
    {
        for (i = 0; i < n; i++)
        {
            x[i] = ((double)(n - i - 1) * x_min + (double)(i)*x_max) / (double)(n - 1);
        }
    }
...
//
{
    ...
    for (i = 0; i < n; i++)
    {
        x[i] = ((double)(n - i) * x_min + (double)(+i + 1) * x_max) / (double)(n + 1);
    }
...
//
{
    ...
    for (i = 0; i < n; i++)
    {
        x[i] = ((double)(2 * n - 2 * i - 1) * x_min + (double)(2 * i + 1) * x_max) / (double)(2 * n);
    }
...
#define N_MAX 11
    ...
    double fx_vec[N_MAX] = {-0.5772156649015329E+00, -0.4237549404110768E+00, -0.2890398965921883E+00,
...
//
{
    ...
    int n;
    double one = 1.0;
    double p[8] = {-1.71618513886549492533811E+00, 2.47656508055759199108314E+01, -3.79804256470945635097577E+02,
                   6.29331155312818442661052E+02,  8.66966202790413211295064E+02, -3.14512729688483675254357E+04,
    ...
    double q[8] = {-3.08402300119738975254353E+01, 3.15350626979604161529144E+02, -1.01515636749021914166146E+03,
                   -3.10777167157231109440444E+03, 2.25381184209801510330112E+04, 4.75584627752788110767815E+03,
    ...
    if (0.75 <= x)
    {
        ...
        if (fabs(c - a - b - (int)(c - a - b)) < 1.0E-15)
        {
            ...
            if (0 <= m)
            {
                ...
                for (k = 1; k <= m - 1; k++)
                {
                    r0 = r0 * (a + k - 1.0) * (b + k - 1.0) / (k * (k - m)) * (1.0 - x);
                    f0 = f0 + r0;
                }

                for (k = 1; k <= m; k++)
                {
                    sp0 = sp0 + 1.0 / (a + k - 1.0) + 1.0 / (b + k - 1.0) - 1.0 / (double)(k);
                }
                ...
                for (k = 1; k <= 250; k++)
                {
                    sp = sp + (1.0 - a) / (k * (a + k - 1.0)) + (1.0 - b) / (k * (b + k - 1.0));

                    sm = 0.0;
                    for (j = 1; j <= m; j++)
                    {
                        sm = sm + (1.0 - a) / ((j + k) * (a + j + k - 1.0)) + 1.0 / (b + j + k - 1.0);
                    }

                    rp = pa + pb + 2.0 * el + sp + sm + log(1.0 - x);

                    r1 = r1 * (a + m + k - 1.0) * (b + m + k - 1.0) / (k * (m + k)) * (1.0 - x);
            ...
            else if (m < 0)
            {
                ...
                for (k = 1; k <= m - 1; k++)
                {
                    r0 = r0 * (a - m + k - 1.0) * (b - m + k - 1.0) / (k * (k - m)) * (1.0 - x);
                    f0 = f0 + r0;
                }
                ...
                for (k = 1; k <= 250; k++)
                {
                    ...
                    r1 = r1 * (a + k - 1.0) * (b + k - 1.0) / (k * (m + k)) * (1.0 - x);
        ...
        else
        {
            ...
            for (k = 1; k <= 250; k++)
            {
                r0 = r0 * (a + k - 1.0) * (b + k - 1.0) / (k * (a + b - c + k)) * (1.0 - x);

                r1 = r1 * (c - a + k - 1.0) * (c - b + k - 1.0) / (k * (c - a - b + k)) * (1.0 - x);
...
//
{
    ...
    int nq;
    double one = 1.0;
    double p1[9] = {4.5104681245762934160E-03, 5.4932855833000385356,     3.7646693175929276856E+02,
                    7.9525490849151998065E+03, 7.1451595818951933210E+04, 3.0655976301987365674E+05,
                    6.3606997788964458797E+05, 5.8041312783537569993E+05, 1.6585695029761022321E+05};
    double p2[7] = {-2.7103228277757834192,    -1.5166271776896121383E+01, -1.9784554148719218667E+01,
                    -8.8100958828312219821,    -1.4479614616899842986,     -7.3689600332394549911E-02,
                    -6.5135387732718171306E-21};
    double piov4 = 0.78539816339744830962;
    double q1[8] = {9.6141654774222358525E+01, 2.6287715790581193330E+03, 2.9862497022250277920E+04,
                    1.6206566091533671639E+05, 4.3487880712768329037E+05, 5.4256384537269993733E+05,
                    2.4242185002017985252E+05, 6.4155223783576225996E-08};
    double q2[6] = {4.4992760373789365846E+01, 2.0240955312679931159E+02, 2.4736979003315290057E+02,
                    1.0742543875702278326E+02, 1.7463965060678569906E+01, 8.8427520398873480342E-01};
    double sgn;
    double three = 3.0;
    double upper;
    double value;
...
//
{
    ...
    for (i = 0; i < n; i++)
    {
        x[i] = -cos(2.0 * r8_pi * (double)(i) / (double)(2 * n - 1));
    }
    xold = new double[n];
    p = new double[n * (n + 1)];
    iterate = 0;
    ...
    do
    {
        ...
        for (i = 1; i < n; i++)
        {
            x[i] = xold[i] - ((1.0 - xold[i]) / (double)(n)) * (p[i + (n - 1) * n] + p[i + n * n]) /
                                 (p[i + (n - 1) * n] - p[i + n * n]);
        }
        test = 0.0;
        for (i = 0; i < n; i++)
...
void scqf(int nt, double t[], int mlt[], double wts[], int nwts, int ndx[], double swts[], double st[], int kind,
