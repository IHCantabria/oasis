...
class BodiesEditor(ListFormEditor):
    ...
    def _item_label(self, item) -> str:
        return item.name if item.name else "Body"
    ...
    def _build_form(self) -> QWidget:
        ...
        self._f_type = make_combo(["RAD_DIFF"])
        ...
        ff = QFormLayout()
        self._f_freedom = make_combo(["0 - Free", "1 - Fixed", "2 - Imposed motion"])
        ...
        self._f_bcp_indices = make_line_edit("")
        ...
        self._f_hdb_index = make_spin(1, 1, 99)
        self._f_take_cog = QCheckBox("Use CoG from HDB")
        self._f_take_cog.setChecked(True)
        self._f_radiation = QCheckBox("Radiation")
        self._f_radiation.setChecked(True)
        self._f_excitation_1st = QCheckBox("1st-order excitation")
        self._f_excitation_1st.setChecked(True)
        ...
        hydrof.addRow("HDB file:", make_file_browse_row(
            ...
            file_filter="HDB files (*.h5 *.hdb5);;All (*)"))
        ...
        self._f_hydrostatics = make_combo(["0 - None", "1 - Mesh STL", "2 - From file"])
        ...
        self._stl_group = QGroupBox("Hydrostatics mesh (STL)")
        ...
        # -- Mass & CoG override ----------------------------------------------
        misc_box = QGroupBox("Mass & CoG override (GUI only)")
        ...
        self._f_sch_height = make_double_spin(5.0, 0.1, 1000, decimals=2)
    ...
    def _populate_form(self, item) -> None:
        ...
        self._f_type.setCurrentText(item.body_type)
        for i, cb in enumerate(self._f_dofs):
            cb.setChecked((i + 1) in item.dofs)
        self._f_freedom.setCurrentIndex(item.freedom_flag)
        self._f_imposed_file.setText(item.imposed_motion_file)
        self._f_bcp_indices.setText(",".join(str(b) for b in item.bcps_indexes))
        ...
        self._f_take_cog.setChecked(bool(item.take_cog_from_hdb))
        self._f_hydrostatics.setCurrentIndex(item.hydrostatics_flag)
        self._f_stl_file.setText(item.hydrostatics_mesh)
        self._f_radiation.setChecked(bool(item.radiation))
        self._f_excitation_1st.setChecked(bool(item.excitation_1st))
        self._f_excitation_2nd.setChecked(bool(item.excitation_2nd))
    ...
    def _read_form(self):
        ...
        item.body_type = self._f_type.currentText()
        item.dofs = [i + 1 for i, cb in enumerate(self._f_dofs) if cb.isChecked()]
        ...
        try:
            item.bcps_indexes = [int(x.strip()) for x in raw.split(",") if x.strip()]
        except ValueError:
            item.bcps_indexes = []
        ...
        item.take_cog_from_hdb = int(self._f_take_cog.isChecked())
        item.hydrostatics_flag = self._f_hydrostatics.currentIndex()
        item.hydrostatics_mesh = self._f_stl_file.text()
        item.radiation = int(self._f_radiation.isChecked())
        item.excitation_1st = int(self._f_excitation_1st.isChecked())
        item.excitation_2nd = int(self._f_excitation_2nd.isChecked())
